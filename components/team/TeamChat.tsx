...
export default function TeamChat({ teamId }: { teamId: string }) {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      // Reverse so the newest is at the bottom of the rendered list
      ...
      channel = client
        ...
        .on(
          ...
          (payload) => {
            ...
            setMessages(prev => {
              // Reconcile by client_message_id: if we have an optimistic entry
              // with the same id, replace it with the server row.
  ...
  return (
    <div className="card">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <p className="eyebrow">PRIVATE TEAM CHAT</p>
                ...
                {m.delivery === 'failed' && (
                  <>
                    ...
                    <button
                      type="button"
                      onClick={() => retry(m)}
                      style={{ background: 'none', border: 0, color: '#ff8b82', cursor: 'pointer', padding: 0, fontSize: 11 }}
                    >
                      failed · tap to retry
                    </button>
                  </>
      ...
      <form onSubmit={send} style={{ display: 'flex', gap: 8 }}>
        ...
        />
        <button className="button" type="submit" disabled={!input.trim()}>Send →</button>
      </form>
      {errorMsg && (
        <p className="muted" style={{ color: '#ff8b82', marginTop: 8, fontSize: 12 }}>{errorMsg}</p>
      )}
    </div>
  )
}
